...
class RandomSplitter(Splitter):
    ...
    def split(self, 
        ...
        np.testing.assert_almost_equal(frac_train + frac_valid + frac_test, 1.0)
        a=[]
        for i in dataset.data_list:
            a.append(i['label'][0])
        a=np.array(a)
        p_label = np.argwhere(a == 1)
        n_label = np.argwhere(a == -1)
        N_p = p_label.shape[0]
        N_n = n_label.shape[0]
        rng = np.random.RandomState(seed)
        rng.shuffle(p_label.squeeze())
        rng.shuffle(n_label.squeeze())
        train_cutoff_p = round(frac_train * N_p)
        train_cutoff_n = round(frac_train * N_n)
        valid_cutoff_p = round((frac_train + frac_valid) * N_p)
        valid_cutoff_n = round((frac_train + frac_valid) * N_n)

        train_cutoff = list(p_label.squeeze()[:train_cutoff_p])+list(n_label.squeeze()[:train_cutoff_n])
        valid_cutoff = list(p_label.squeeze()[train_cutoff_p:valid_cutoff_p])+list(n_label.squeeze()[train_cutoff_n:valid_cutoff_n])
        test_cutoff = list(p_label.squeeze()[valid_cutoff_p:])+list(n_label.squeeze()[valid_cutoff_n:])

        train_dataset = dataset[train_cutoff]
        valid_dataset = dataset[valid_cutoff]
        test_dataset = dataset[test_cutoff]
